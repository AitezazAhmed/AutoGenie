...
const Hashtaggenerator = () => {
      ...
      const { generatedhashtag, hashtaggenerator } = useResponseStore();

      const handleGenerate = () => {
    hashtaggenerator(input); // call backend
  };
  return (
      ...
      <h1 className="text-3xl font-bold text-sky-600 mb-2">
        AutoGenie - Hashtag Generator
      </h1>
      <p className="text-gray-500 mb-6">Generate hashtag instantly using AI</p>
      ...
      {generatedhashtag && (
        <div className="mt-6 w-full max-w-2xl bg-sky-50 border border-sky-200 rounded-xl p-4 shadow">
          ...
          <p className="text-gray-700">{generatedhashtag}</p>
        </div>
      )}
    </div>
  )
